...
if __name__ == "__main__":
    ...
    x = [xinit_/2 for xinit_ in xinit]
    y = [yinit_/2 for yinit_ in yinit]
    yaw = [np.pi, -np.pi/3, np.pi/3]
    k = 0
    ...
    timeHelper.sleep(1)
    for cf in allcfs.crazyflies:
        cf.goTo(np.array([xinit[k], yinit[k], initHeight + 0.2]), yaw[k], 2)
        k = k+1
    k = 0
    timeHelper.sleep(1)
    ...
    for cf in allcfs.crazyflies:
        ...
        cf.goTo(np.array([x[k], y[k], initHeight + 0.2]), yaw[k], 2)
        k = k+1
    k = 0
    timeHelper.sleep(3)
