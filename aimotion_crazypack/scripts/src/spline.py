...
class BSpline(Spline):
    ...
    def __add__(self, other):
        ...
        # elif get_module(self.coeffs) in ['gurobipy']:
        #     basis = self.basis
            
            # coeffs = {}
            # for i in range(len(self.basis)):
            #     coeffs[i] = self.coeffs[i] + other
        else:
            try:
                basis = self.basis
    ...
    def __mul__(self, other):
        if isinstance(other, self.__class__):
            ...
            try:
                ...
                #         coeffs_product += [self.coeffs[pairs[0][i]] * other.coeffs[pairs[1][i]]]
                #     coeffs = T.dot(coeffs_product)
                #     coeffs_dict = {}
