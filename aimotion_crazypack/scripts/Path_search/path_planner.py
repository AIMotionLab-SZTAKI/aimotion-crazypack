...
import numpy as np
from scripts.Path_search.read_optitrack import read_optitrack
import os
...
def main2(scene, mobs_obj, V0):
    ...
    Nv = 150                                     # Maximal number of vertices to be generated
    thres = 0.1
    seed = 440
...
def run_main(select = False):
    ...
    if select == False or select == None:
        # Random position
        _, drone1_xf = random_V0_generator(x_lim = scene.dims[:2], y_lim = scene.dims[2:], d_min = 1.0, p0 = drone1_x0, d_max = 1.5)
    ...
    if select == False or select == None:
        _, drone2_xf = random_V0_generator(x_lim = scene.dims[:2], y_lim = scene.dims[2:], d_min = 1.0, p0 = drone2_x0, d_max = 1.5, p2_clear = drone1_xf)
