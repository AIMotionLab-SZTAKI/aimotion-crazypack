...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    # MOVING OBSTACLES
    obs_paths = np.array([0])                   # Give the indexes of the paths for the moving obstacles
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    if Random_SE:
        ...
        a_point = [19,23,27,31]
        b_point = [16,20,24,28]
        predef_SE = [[[a_point[0], b_point[0]],[a_point[1], b_point[1]],
                      [a_point[2], b_point[2]], [a_point[3], b_point[3]]]]  # 0

        predef_SE.append([[b_point[0], a_point[0]],[b_point[1], a_point[1]],
                      [b_point[2], a_point[2]], [b_point[3], a_point[3]]])  # 1
        predef_SE.append([[a_point[0], b_point[0]],[a_point[1], b_point[1]],
                      [a_point[2], b_point[2]], [a_point[3], b_point[3]]])  # 2
    ...
    #predef_index = 0 # rewrite index to constant
    #delay_sim = 0
# ----------------------------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------------------
    # MOVING DUMMIES
    idx_dummy = np.array([0])  # The indexes of the mob paths
    dummies_r = np.array([0.1])  # The radius of the mobs
...
# ......................................................................................................................
    ...
    measured = [[],[],[],[]]
    col_drone = ["tab:red", "tab:blue", "tab:green", "tab:orange"]
    if measurement_plot:
        fig = plt.figure()
        delay_sim = 0
        for i in range(2):
            #if i == predef_index:
                pickle_in = open("Drone_positions"+"{:}".format(i+1)+".00000.pickle", "rb")
                measured_pos = pickle.load(pickle_in)
                pickle_in.close()
                measured[0] = np.array(measured_pos[0])
                measured[1] = np.array(measured_pos[1])
                measured[2] = np.array(measured_pos[2])
                measured[3] = np.array(measured_pos[3])
                for j in range(len(mobs_obj)-drone_num, len(mobs_obj)):
                    #if not((i == 1 and j == 2) or (i == 3 and j == 2)):
                        time_measure = [max(x-delay_sim, 0) for x in measured[j-(len(mobs_obj)-drone_num)][1:, 0]]
                        start_plot = [x for x in time_measure if x != 0]
                        start_plot = len(time_measure) - len(start_plot)
                        print(start_plot)
                        x_s, y_s, z_s = interpolate.splev(mobs_obj[j].move(time_measure), mobs_obj[j].path)
                        sim = np.array([x_s, y_s, z_s])
                        sim = np.transpose(sim)
                        #dist = linalg.norm(sim - measured[j-(len(mobs_obj)-drone_num)][1:, 1:], axis=1, ord=2)
                        #plt.plot(time_measure[start_plot:], dist[start_plot:], lw=2, color=col_drone[j-(len(mobs_obj)-drone_num)])
                        #dist_x = sim[:,0] - measured[j - (len(mobs_obj) - drone_num)][1:, 1] # only one direction
                        #plt.plot(time_measure[start_plot:], dist_x[start_plot:])
                        dist_y = sim[:, 1] - measured[j - (len(mobs_obj) - drone_num)][1:, 2]  # only one direction
                        plt.plot(time_measure[start_plot:], dist_y[start_plot:], lw=1, label="y error", color="tab:orange")
                        dist_z = sim[:, 2] - measured[j - (len(mobs_obj) - drone_num)][1:, 3]  # only one direction
                        plt.plot(time_measure[start_plot:], dist_z[start_plot:], lw=1, label="z error", color="tab:blue")
        #plt.legend()
        plt.legend(["y error","z error"], bbox_to_anchor=(1, 1), loc=1, borderaxespad=0)
        plt.xlabel("time [sec]")
